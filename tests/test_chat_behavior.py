...
def test_resolve_session_id_falls_back_to_user_default():
    ...
    assert resolve_session_id("session-abc", "user-123") == "session-abc"
...
def test_mock_interview_prompt_uses_cv_and_does_not_start_with_self_intro():
    ...
    assert "câu hỏi đầu tiên" in lowered
    assert "dựa vào cv" in lowered or "dựa vào cv thực tế" in lowered
